...
function penci_register_recipe_print_scripts() {
	...
	wp_register_script( 'penci_rateyo', plugin_dir_url( __FILE__ ) . 'js/rating_recipe.js', array( 'jquery' ), PENCI_RECIPE_VER, true );
	$localize_script = array(
		'ajaxUrl'     => admin_url( 'admin-ajax.php' ),
		'nonce'       => wp_create_nonce( 'ajax-nonce' ),
		'ajax_review' => (bool) get_theme_mod( 'penci_recipe_review_ajax', true ),
	);
	wp_localize_script( 'penci_rateyo', 'PENCI', $localize_script );

	if ( ! get_theme_mod( 'penci_recipe_rating' ) ) {
		wp_enqueue_script( 'penci_rateyo' );
	}
}
...
if ( ! function_exists( 'penci_rateyo' ) ) {
	...
	function penci_rateyo() {
		...
		$postid    = $_POST['postid'];
		$rating    = $_POST['rating'];
		$postidsub = isset( $_POST['postidsub'] ) && $_POST['postidsub'] ? $_POST['postidsub'] : '';

		$rate_total_meta  = 'penci_recipe_rate_total';
		$rate_people_meta = 'penci_recipe_rate_people';
		$cookie_name      = 'recipe_rate_postid_' . $postid;

		if ( $postidsub ) {
			$rate_total_meta  = 'penci_recipe_rate_total';
			$rate_people_meta = 'penci_recipe_rate_people';
			$cookie_name      = 'recipe_rate_postid_' . $postid . $postidsub;
		}

		if ( $postid && $rating ) {

			// Get rate meta in post
			$rate_total  = get_post_meta( $postid, $rate_total_meta, true );
			$rate_people = get_post_meta( $postid, $rate_people_meta, true );

			// Update rate meta to post
			$update_rate_total = intval( $rating ) + intval( $rate_total );
			update_post_meta( $postid, $rate_total_meta, $update_rate_total );
			$update_rate_people = intval( $rate_people ) + 1;
			update_post_meta( $postid, $rate_people_meta, $update_rate_people );

			setcookie( $cookie_name, rand(), time() + ( 86400 * 30 ), "/" );
			wp_send_json_success( array(
				'success' => true,
				'total'   => $update_rate_total,
				'people'  => $update_rate_people
			) );
...
if ( ! function_exists( 'penci_get_recipe_rating' ) ) {
	add_action( 'wp_ajax_nopriv_penci_get_recipe_rating', 'penci_get_recipe_rating' );
	add_action( 'wp_ajax_penci_get_recipe_rating', 'penci_get_recipe_rating' );

	function penci_get_recipe_rating() {
		$rate_total = $rate_people = 0;
...
add_action( 'elementor/widgets/register', function ( $widgets_manager ) {
	...
	require_once( plugin_dir_path( __FILE__ ) . 'inc/elementor-buttons.php' );
	$widgets_manager->register( new \PenciRecipeElementor() );
	$widgets_manager->register( new \PenciRecipeButtonsElementor() );
} );