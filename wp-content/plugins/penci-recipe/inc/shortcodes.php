...
function penci_recipe_shortcode_function( $atts, $content = null ) {
	...
	$rate_total  = (int) get_post_meta( $recipe_id, 'penci_recipe_rate_total', true );
	$rate_people = (int) get_post_meta( $recipe_id, 'penci_recipe_rate_people', true );

	// Set default values if they are empty or not set
	if ( empty( $rate_total ) ) {
		$rate_total = 5;
		update_post_meta( $recipe_id, 'penci_recipe_rate_total', $rate_total );
	}

	if ( empty( $rate_people ) ) {
		$rate_people = 1;
		update_post_meta( $recipe_id, 'penci_recipe_rate_people', $rate_people );
	}
            ...
            <div class="penci-recipe-heading">
                ...
                <div class="penci-recipe-metades">
					...
					<?php if ( ! get_theme_mod( 'penci_recipe_rating' ) && $flag_style == true ) : ?>
                        <div class="penci-recipe-rating penci-recipe-review" data-recipe-id="<?php echo $recipe_id; ?>">
                        ...
                        <article id="post-<?php the_ID(); ?>" <?php post_class( 'penci-recipe-item' ); ?>>
							<?php if ( $index_display_image != 'no' && function_exists( 'penci_get_featured_image_size' ) ) : ?>
                                <div class="penci-index-post-img">
									<?php
									// Define allowed image sizes (adjust to match your theme's options)
									$allowed_image_sizes = array( 'vertical', 'horizontal', 'square' );

									// Validate $index_image_size
									$index_image_size = ( isset( $index_image_size ) && in_array( $index_image_size, $allowed_image_sizes, true ) )
										? $index_image_size
										: 'vertical';
									$thumbnail_size = 'penci-thumb-square';
